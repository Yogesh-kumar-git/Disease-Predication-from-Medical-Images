...
A deep learning-based web application that predicts diseases from medical images using Convolutional Neural Networks (CNNs). The system currently supports **Pneumonia detection** from chest X-rays and **Brain Tumour detection** from MRI scans — with a user-friendly Streamlit interface where users can simply upload their scan and get an instant prediction.
